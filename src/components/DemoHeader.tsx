...
export const DemoHeader: React.FC<DemoHeaderProps> = ({
  ...
  return (
    <div className="bg-gradient-to-r from-[#143151] to-[#387E89] text-white py-3 px-3 sm:py-4 sm:px-4 border-b border-white/10 shadow-lg">
      <div className="flex flex-col space-y-2">
        <div className="flex justify-between items-center">
          ...
          <button 
            onClick={togglePause} 
            className="bg-white/30 hover:bg-white/40 rounded-full p-2 sm:p-2.5 md:p-3 transition-all shadow-lg border border-white/30 group cursor-pointer focus:outline-none focus:ring-2 focus:ring-white/50 flex-shrink-0" 
          ...
          >
            {isPaused ? 
              <Play size={isMobile ? 20 : 24} className="text-white group-hover:text-white/90" /> : 
              <Pause size={isMobile ? 20 : 24} className="text-white group-hover:text-white/90" />
            }
          </button>
        </div>
        ...
        <Tabs value={currentStage.toString()} onValueChange={value => handleStageChange(parseInt(value))} className="w-full">
          <TabsList className="w-full bg-white/10 p-1 sm:p-1.5 rounded-md border border-white/20 overflow-x-auto flex flex-nowrap no-scrollbar scroll-pl-3 scroll-smooth">
            {stageDescriptions.map((stage, index) => (
              <TabsTrigger 
                key={index} 
                value={index.toString()} 
                className={`
                  min-w-max flex-grow flex items-center justify-center gap-2 
                  ${currentStage === index ? 
              ...
              >
                <span className={`
                  ...
                  ${isMobile ? 'h-6 w-6 sm:h-7 sm:w-7' : 'h-6 w-6 sm:h-8 sm:w-8'} 
                  rounded-full 
                ...
                <span className="hidden xxs:inline xxs:max-w-[70px] xs:max-w-[90px] sm:max-w-none truncate">
                  {isMobile ? 
                    (index === 0 ? "Patient" : 
                     index === 1 ? "Scribe" : 
                     index === 2 ? "Admin" : "Care") : 
