...
export const DemoHeader: React.FC<DemoHeaderProps> = ({
  ...
  return <div className="bg-gradient-to-r from-[#143151] to-[#387E89] text-white py-3 px-4 sm:py-4 sm:px-6 border-b border-white/10 shadow-lg">
      <div className="flex flex-col space-y-2">
        <div className="flex justify-between items-center">
          <h3 className="font-bold text-xl sm:text-2xl md:text-3xl">AI-Automated Clinical Workflows Demo</h3>
        ...
        <Tabs value={currentStage.toString()} onValueChange={value => handleStageChange(parseInt(value))} className="w-full">
          <TabsList className="w-full bg-white/10 p-1 rounded-md border border-white/20">
            {stageDescriptions.map((stage, index) => <TabsTrigger key={index} value={index.toString()} className={`flex-1 flex items-center justify-center gap-2 ${currentStage === index ? 'bg-white/20 text-white shadow-lg' : 'text-white/80 hover:bg-white/15 hover:text-white'} 
                ...
                <span className={`inline-flex items-center justify-center h-6 w-6 sm:h-7 sm:w-7 rounded-full 
                  ${currentStage === index ? 'bg-[#FFDEE2] text-[#143151]' : 'bg-white/20 text-white'} text-sm sm:text-base font-bold`}>
                  {index + 1}
                </span>
