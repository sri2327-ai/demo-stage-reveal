...
export const DemoStage: React.FC<DemoStageProps> = ({
  ...
  return <div ref={demoContainerRef} className="relative w-full h-[540px] sm:h-[620px] md:h-[720px] lg:h-[780px] bg-white rounded-2xl shadow-xl overflow-hidden border border-[#387E89]/10" role="region" aria-label="Interactive clinical workflow demonstration" tabIndex={0}>
      {/* Enhanced header with tabbed navigation */}
      <motion.div className="absolute top-0 left-0 right-0 bg-gradient-to-r from-[#143151] to-[#387E89] text-white py-4 px-4 sm:py-5 sm:px-6 z-40 border-b border-white/10 shadow-lg" initial={{
      y: -100
    }} animate={{
      y: 0
    }} transition={{
      ...
      damping: 30
        ...
        <div className="flex flex-col space-y-2">
          <div className="flex justify-between items-center">
            <motion.h3 className="font-bold text-xl sm:text-2xl md:text-3xl" initial={{
            opacity: 0,
            x: -20
          }} animate={{
            opacity: 1,
            ...
            duration: 0.5
          }}>AI Clinical Workflows Demo</motion.h3>
            
            <button onClick={togglePause} className="bg-white/30 hover:bg-white/40 rounded-full p-2 sm:p-2.5 md:p-3 transition-all shadow-lg border border-white/30 group cursor-pointer focus:outline-none focus:ring-2 focus:ring-white/50" aria-label={getPlayPauseAriaLabel()}>
              {isPaused ? <Play size={isMobile ? 18 : 22} className="text-white group-hover:text-white/90" /> : <Pause size={isMobile ? 18 : 22} className="text-white group-hover:text-white/90" />}
            </button>
          </div>
          ...
          <Tabs value={currentStage.toString()} onValueChange={value => handleStageChange(parseInt(value))} className="w-full">
            <TabsList className="w-full bg-white/10 p-1 rounded-md border border-white/20">
              {stageDescriptions.map((stage, index) => <TabsTrigger key={index} value={index.toString()} className={`flex-1 ${currentStage === index ? 'bg-white/20 text-white' : 'text-white/80 hover:text-white'} 
          ...
          <motion.p className="text-sm sm:text-base text-white/90 max-w-xl line-clamp-2 mb-1" key={`desc-${currentStage}`} initial={{
          opacity: 0,
          y: 5
        }} animate={{
          opacity: 1,
      ...
      <AnimatePresence mode="wait">
        <motion.div key={currentStage} className="absolute inset-0 pt-[120px] sm:pt-[140px] md:pt-[150px]" initial={{
        opacity: 0,
        scale: 0.9
      }} animate={{
        opacity: 1,
      ...
      <DemoStageIndicator currentStage={currentStage} totalStages={stages.length} onStageChange={handleStageChange} isDemoSection={isDemoSection} />
      
      {/* Enhanced interactive indicator - more prominent and accessible */}
      <AnimatePresence>
        {highlightInteractivity && <motion.div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-40 pointer-events-none" initial={{
        opacity: 0,
        scale: 0.8
      }} animate={{
        opacity: [0.7, 1, 0.7],
