...
const MedicalPhrases = () => {
  ...
  const filteredItems = useMemo(() => {
    return mockMedicalPhrases.filter(item => {
      const matchesSearch = item.phrase.toLowerCase().includes(searchTerm.toLowerCase()) || item.meaning.toLowerCase().includes(searchTerm.toLowerCase()) || item.clinicalContext.toLowerCase().includes(searchTerm.toLowerCase()) || item.relatedPhrases.some(phrase => phrase.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12 max-w-7xl">
        ...
        <div className="mb-8 sm:mb-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
            ...
            return <Card key={specialty.id} className={`cursor-pointer transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${isSelected ? 'ring-2 ring-[#387E89] shadow-lg bg-gradient-to-br from-[#387E89]/5 to-[#143151]/5' : 'hover:shadow-lg'}`} onClick={() => handleSpecialtyFilter(specialty.name)}>
        ...
        <div className="mb-8 sm:mb-12">
          <div className="max-w-2xl mx-auto px-4">
            <div className="relative group">
              ...
              <Input type="text" placeholder="Search medical phrases, meanings, contexts..." value={searchTerm} onChange={e => {
        ...
        <div className="mb-12 sm:mb-16">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 sm:mb-8 gap-3 sm:gap-4 px-4 sm:px-0">
            <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-[#143151]">
              Medical Phrases
              {(selectedSpecialty || selectedSection || selectedLetter) && <span className="ml-2 sm:ml-3 text-[#387E89] text-lg sm:text-xl lg:text-2xl">
          ...
          {currentItems.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 px-4 sm:px-0">
              {currentItems.map(item => <Link key={item.id} to={`/medical-phrases/${item.id}`} className="group block">
          ...
          {/* Pagination */}
          {totalPages > 1 && <div className="flex justify-center mt-8 sm:mt-12 px-4">
              <Pagination>
                <PaginationContent>
                  <PaginationItem>
